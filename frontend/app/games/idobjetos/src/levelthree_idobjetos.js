var levelthree_idobjetos = function(game){
	var leftArrow;
	var errorScreen;
	var fx;
	var sound;
...
levelthree_idobjetos.prototype = {
  	create: function(){
  		intentos = 0;
		// showing level title
		var style = {
		...
		fx = this.game.add.audio('objetos');
    	fx.allowMultiple = true;

		fx.addMarker('reloj', 0, 2.0);
		fx.addMarker('botella', 2, 1.9);
		fx.addMarker('hoja', 3.8, 1.8);
		fx.addMarker('casa', 5.6, 1.6);
		fx.addMarker('taza', 7.2, 1.7);
		fx.addMarker('portafolios', 8.9, 1.8);
		fx.addMarker('pelota', 10.7, 1.8);
		fx.addMarker('foto', 12.5, 2.5);
		fx.addMarker('telefono', 15.1, 1.7);
		fx.addMarker('disco', 16.8, 1.9);
		fx.addMarker('impresora', 18.5, 2.0);
		fx.addMarker('collar', 20.5, 1.8);

		var shapeSounds=['reloj', 'botella', 'hoja', 'casa',
		'taza', 'portafolios', 'pelota', 'foto',
		'telefono', 'disco', 'impresora', 'collar',];
