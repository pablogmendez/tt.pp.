var levelnineteen_completafrases = function(game){
	var leftArrow;
	var errorScreen;
	var fx;
	var sound;
...
levelnineteen_completafrases.prototype = {
  	create: function(){
  		intentos = 0;
  		aciertos = 0;
		// showing level title
		...
		var gameThumb = this.game.add.sprite(0, 25, "frase19");

		var position = 370;
		var posX = [this.game.width/2 -70, this.game.width/2 +70, this.game.width/2 -70, this.game.width/2 +70];
		var posY = [ position, position, position+30, position+30];
		var coord = [0, 1, 2, 3];

		// Telefono this.game.width/2 -110, 110
		// Camino this.game.width/2 +80, 110
		// Amigo this.game.width/2 -110, 300
		var gameThumb11 = this.game.add.sprite(this.game.width/2 -110, 150, "sprites19_fade");
		gameThumb11.anchor.setTo(0.5);
		gameThumb11.frame = 1;
		this.game.physics.arcade.enable(gameThumb11);
		gameThumb11.tint= 0x000000;

		var gameThumb12 = this.game.add.sprite(this.game.width/2 -70, 45, "sprites19_fade");
		gameThumb12.anchor.setTo(0.5);
		gameThumb12.frame = 2;
		this.game.physics.arcade.enable(gameThumb12);
		gameThumb12.tint= 0x000000;

		var gameThumb13 = this.game.add.sprite(this.game.width/2+10, 150, "sprites19_fade");
		gameThumb13.anchor.setTo(0.5);
		gameThumb13.frame = 3;
		this.game.physics.arcade.enable(gameThumb13);
		gameThumb13.tint= 0x000000;

		var gameThumb14 = this.game.add.sprite(this.game.width/2-120, 255, "sprites19_fade");
		gameThumb14.anchor.setTo(0.5);
		gameThumb14.frame = 4;
		this.game.physics.arcade.enable(gameThumb14);
		gameThumb14.tint= 0x000000;

		var gameThumb1 = this.game.add.sprite(posX[coord[0]], posY[coord[0]], "sprites19");
		var gameThumb2 = this.game.add.sprite(posX[coord[1]], posY[coord[1]], "sprites19");
		var gameThumb3 = this.game.add.sprite(posX[coord[2]], posY[coord[2]], "sprites19");
		var gameThumb4 = this.game.add.sprite(posX[coord[3]], posY[coord[3]], "sprites19");
	...
	playPhrase: function() {
		fx2 = this.game.add.audio('fase4');
    	fx2.allowMultiple = true;
		fx2.addMarker('resp', 33.6, 13.0);
